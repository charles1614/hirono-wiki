...
- GB NVL72 + NVFP4 delivers 50× token throughput or 35× token cost reduction vs H200 NVL8 at same power (DeepSeek-R1-0528 FP4, 1K/1K I/O); critically, most of this gain comes from larger NVLink domain + concurrency headroom from bigger VRAM, not raw FP compute (B300 vs H100 FP8 is only ~2×). NVIDIA Dynamo TRT 1-month optimization achieved large throughput gains on GB200 NVL72 per SemiAnalysis. — [[2026-03-18-nvidia-gtc2026-详细解读和分析]]
...
- [[UMMA]] (`tcgen05.mma`) replaces Hopper WGMMA on Blackwell; each SM has 256 KB [[Tensor Memory]] (TMEM) for accumulator storage (eliminating register pressure for MMA); max UMMA atom is 128×256×16 (2× WGMMA); single-thread launch model; CTA-pair mode spans 2 SMs; consumer Blackwell (CC 12.0, e.g. 5090d) lacks TMEM and cannot use UMMA. — [[2025-06-09-一起聊聊nvidia-blackwell-新特性之umma]]
- GPU performance first-principles analysis: computation density (FLOPS/Data rate) has declined each generation as memory bandwidth fails to keep pace; SM2SM network (DCMEM) with TMA-based L1-cache sharing reduces L2 latency by 32% (180 cycle SM2SM vs ~265 cycle L2); sparse Tensor Core splits sparse matrix into non-zero values + indices to raise compute density. — [[2025-06-09-gpu性能的第一性原理]]
