...
![Figure 1: Model of the NVIDIA Hopper H100 GPU](01-h100_model.png)
...
![Figure 2: Memory hierarchy of the H100 (SXM5) GPU](02-mem_hierarchy.png)
...
![Figure 3: H100 SXM5 BF16 speed-of-light derivation](03-h100_sol.png)
...
![Figure 4: Power throttling reduces clock frequency and lowers the effective “speed of light”](04-clk.png)
...
![Figure 5: CUDA Programming Model: threads, warps, blocks, clusters, grids](05-cuda_model.png)
...
![Figure 6: CUDA's built-in variables: how threads know where they are](06-cuda_model2.png)
...
![Figure 7: Inside a DRAM cell: transistor + capacitor, wordline + bitline](07-gmem_dram_cell.png)
...
![Figure 8: GMEM model](08-gmem.png)
...
![Figure 9: Effect of access pattern in GMEM](09-gmem_example.png)
...
![Figure 10: SMEM model](10-smem_pt1.png)
...
![Figure 11: SMEM: good vs. bad access patterns](11-smem_pt2.png)
...
![Figure 12: SMEM: multicasting (served in a single cycle)](12-smem_pt3.png)
...
![Figure 13: L1 cache model](13-l1.png)

Figure 13: L1 cache model

Let's go one level deeper and look at the tag stage and data stage in detail:

![Figure 14: Breakdown of k-way set-associative cache organization](14-kway.png)
...
![Figure 15: CUDA compilation flow: from CUDA C++ → PTX → SASS](15-ptx_sass_hierarchy.png)
...
![Figure 16: Naive CPU matmul example](16-cpu_kernel_example.png)
...
![Figure 17: Tile quantization](17-tile_quantization.png)
...
![Figure 18: Thread organization in naive matmul kernel](18-simple_kernel_logical.png)

Figure 18: Thread organization in naive matmul kernel

And the matmul logic itself looks like this:

![Figure 19: Naive matmul kernel](19-simple_kernel.png)
...
![Figure 20: Nsight Compute: Occupancy, Waves info](20-occupancy_waves.png)
...
![Figure 21: Wave quantization](21-wave_quant.png)
...
![Figure 22: PTX code corresponding to naive matmul CUDA kernel](22-ptx.png)
...
![Figure 23: SASS code corresponding to naive matmul CUDA kernel](23-sass.png)
...
![Figure 24: New logical organization of row2 and col2 variables](24-simple_kernel_uncoalesced_logical.png)

Figure 24: New logical organization of row2 and col2 variables

And here's what the modified kernel does now:

![Figure 25: Naive kernel with uncoalesced GMEM access](25-simple_kernel_uncoalesced.png)
...
![Figure 26: Roofline model](26-roofline.png)
...
![Figure 27: Matmul as a sum of partial outer products](27-outer.png)
...
![Figure 28: Dot product is equivalent to a sum of partial dot products](28-block_tile_equivalence.png)
...
![Figure 29: Arithmetic intensity improves when each thread computes multiple outputs and when tiles approach a square shape](29-arithmetic_intensity.png)
...
![Figure 30: High-level structure of the warp-tiling algorithm, also referred to as block tiling.](30-warp_tiling_pt1.png)
...
![Figure 31: Loading a chunk of B (GMEM) into Bs (SMEM)](31-warp_tiling_pt2_smem_load_b.png)
...
![Figure 32: Loading a chunk of A (GMEM) into As (SMEM)](32-warp_tiling_pt2_smem_load.png)
...
![Figure 33: Performing matmul between As and Bs as a series of thread-level outer products (warp-tiling + thread-tiling).](33-warp_tiling_pt3_compute.png)
...
![Figure 34: We keep the same high-level structure of the warp-tiling algorithm (block-tiling).](34-high_level_block_structure.png)
...
![Figure 35: Swizzling example](35-swizzle_example.png)
...
![Figure 36: No bank conflicts when loading rows or columns](36-swizzle_property.png)
...
![Figure 37: Swizzle function intuition](37-swizzle_function_intuition.png)
...
![Figure 38: Connecting the swizzle function to the matrix swizzle example](38-swizzle_apply.png)
...
![Figure 39: Why doing four 64x16 @ 16x64 wgmma calls is equivalent to doing a 64x64 @ 64x64 matmul](39-4_wgmma_calls.png)
...
![Figure 40: We are wasting TMA and TC cycles - we can do better](40-better_pipeline.png)
...
![Figure 41: More efficient TC/TMA pipeline: producer warp-group streams tiles into a circular buffer; consumer warp-group drains tiles into tensor cores.](41-kernel4_pipeline.png)
...
![Figure 42: Two consumer warp groups let us grow the tile from 128x128 -> 128x256 without register spills](42-2wgs.png)
...
![Figure 43: Persistent kernels: overlap the output store with incoming loads by launching one long-lived block per SM that processes many tiles.](43-overlap_stores.png)
...
![Figure 47: Using thread block clusters to reduce the number of L2/GMEM loads.](47-clusters.png)
...
Baseline (warp-tiling) → Tensor Cores + TMA

32

317

Increase output tile size

317

423

Pipeline: overlap TMA loads with TC compute

423

498

Tile growth: 128×128 → 128×256 (2 consumer warp-groups)
...
TMA async stores (regs → SMEM → GMEM)
