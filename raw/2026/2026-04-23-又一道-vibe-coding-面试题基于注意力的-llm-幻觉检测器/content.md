...
## [](#The-Challenge-基于注意力的-LLM-幻觉检测器 "The Challenge: 基于注意力的 LLM 幻觉检测器")The Challenge: 基于注意力的 LLM 幻觉检测器

#### [](#1-背景与问题-Background-Problem-Statement "1. 背景与问题 (Background & Problem Statement)")**1\. 背景与问题 (Background & Problem Statement)**
...
#### [](#2-Transformer核心机制：数据库查询的视角 "2. Transformer核心机制：数据库查询的视角")**2\. Transformer核心机制：数据库查询的视角**
...
#### [](#3-你的任务-Your-Mission "3. 你的任务 (Your Mission)")**3\. 你的任务 (Your Mission)**
...
#### [](#4-核心挑战与约束-Core-Challenge-Constraints "4. 核心挑战与约束 (Core Challenge & Constraints)")**4\. 核心挑战与约束 (Core Challenge & Constraints)**
...
#### [](#5-可视化要求-Visualization-Requirements "5. 可视化要求 (Visualization Requirements)")**5\. 可视化要求 (Visualization Requirements)**
...
## [](#The-Journey-可视化驱动的发现之旅 "The Journey: 可视化驱动的发现之旅")The Journey: 可视化驱动的发现之旅

### [](#第一步：创建测试用例，明确实现框架 "第一步：创建测试用例，明确实现框架")第一步：创建测试用例，明确实现框架
...
### [](#第二步：先建立”眼睛”——可视化系统 "第二步：先建立”眼睛”——可视化系统")第二步：先建立”眼睛”——可视化系统
...
### [](#第三步：观察模式——让数据自己说话 "第三步：观察模式——让数据自己说话")第三步：观察模式——让数据自己说话
...
[![非幻觉场景 1: 测试用例，输出事实](01-attention1.png)](01-attention1.png "非幻觉场景 1: 测试用例，输出事实")非幻觉场景 1: 测试用例，输出事实
[![非幻觉场景 1: 热力图左侧部分，system prompt 部分有明显的注意力峰值](02-attention2.png)](02-attention2.png "非幻觉场景 1: 热力图左侧部分，system prompt 部分有明显的注意力峰值")非幻觉场景 1: 热力图左侧部分，system prompt 部分有明显的注意力峰值
...
[![幻觉场景 1: 测试用例，输出幻觉](03-attention5.png)](03-attention5.png "幻觉场景 1: 测试用例，输出幻觉")幻觉场景 1: 测试用例，输出幻觉
[![幻觉场景 1: 热力图左侧部分，除了 system prompt 开头，注意力都很低](04-attention6.png)](04-attention6.png "幻觉场景 1: 热力图左侧部分，除了 system prompt 开头，注意力都很低")幻觉场景 1: 热力图左侧部分，除了 system prompt 开头，注意力都很低
[![幻觉场景 1: 热力图右侧部分，user prompt 部分有一些不算高的注意力峰值](05-attention7.png)](05-attention7.png "幻觉场景 1: 热力图右侧部分，user prompt 部分有一些不算高的注意力峰值")幻觉场景 1: 热力图右侧部分，user prompt 部分有一些不算高的注意力峰值

> **我的洞察：** “看到了吗？当模型说真话时，它会’回头看’系统提示；当它说谎时，它无处可看！”

### [](#第四步：从观察到算法——提炼核心信号 "第四步：从观察到算法——提炼核心信号")第四步：从观察到算法——提炼核心信号
...
### [](#第五步：调试和优化——像老师一样纠错 "第五步：调试和优化——像老师一样纠错")第五步：调试和优化——像老师一样纠错
...
[![非幻觉场景 1: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力](05-attention7.png)](05-attention7.png "非幻觉场景 1: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力")非幻觉场景 1: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力
[![幻觉场景 1: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力](06-attention3.png)](06-attention3.png "幻觉场景 1: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力")幻觉场景 1: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力

比较上述幻觉和非幻觉场景，可以发现，注意力热力图的右侧部分（自回归生成的 token 跟它之前生成的 token 之间）总是有较强的注意力。因此，在检测幻觉时，必须排除生成部分的自注意力，而要关注生成部分与输入上下文的交叉注意力。

### [](#第六步：完善可视化——让洞察更清晰 "第六步：完善可视化——让洞察更清晰")第六步：完善可视化——让洞察更清晰
...
[![非幻觉场景 1: 每个生成 token 在 system prompt 区域的注意力曲线](07-attention4.png)](07-attention4.png "非幻觉场景 1: 每个生成 token 在 system prompt 区域的注意力曲线")非幻觉场景 1: 每个生成 token 在 system prompt 区域的注意力曲线
[![幻觉场景 1: 每个生成 token 在 system prompt 区域的注意力曲线](08-attention8.png)](08-attention8.png "幻觉场景 1: 每个生成 token 在 system prompt 区域的注意力曲线")幻觉场景 1: 每个生成 token 在 system prompt 区域的注意力曲线

### [](#关键洞察：可视化是发现的眼睛 "关键洞察：可视化是发现的眼睛")关键洞察：可视化是发现的眼睛
...
## [](#最终成果：基于注意力峰值的简化检测系统 "最终成果：基于注意力峰值的简化检测系统")最终成果：基于注意力峰值的简化检测系统
...
#### [](#极简版-LogitsProcessor-实现 "极简版 LogitsProcessor 实现")极简版 LogitsProcessor 实现
...
2.  **峰值检测逻辑**：
    ...
    for position in generated\_positions:
        attention = get\_attention\_weights(position)

        max\_system\_attention = max(attention\[5:system_prompt_length])
...
### [](#可视化方案：注意力峰值检测看板 "可视化方案：注意力峰值检测看板")**可视化方案：注意力峰值检测看板**

为了直观展示峰值检测系统的工作过程，我们设计了一个清晰的可视化看板，重点展示系统提示区域的注意力峰值。

#### [](#1-注意力热力图-Attention-Heatmap "1. 注意力热力图 (Attention Heatmap)")**1\. 注意力热力图 (Attention Heatmap)**
...
#### [](#3-验证结果分析面板 "3. 验证结果分析面板")**3\. 验证结果分析面板**
...
[![非幻觉场景 2: 测试用例，输出事实](09-attention9.png)](09-attention9.png "非幻觉场景 2: 测试用例，输出事实")非幻觉场景 2: 测试用例，输出事实
[![非幻觉场景 2: 热力图左侧部分，system prompt 区域有明显的注意力峰值](10-attention10.png)](10-attention10.png "非幻觉场景 2: 热力图左侧部分，system prompt 区域有明显的注意力峰值")非幻觉场景 2: 热力图左侧部分，system prompt 区域有明显的注意力峰值
[![非幻觉场景 2: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力](11-attention11.png)](11-attention11.png "非幻觉场景 2: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力")非幻觉场景 2: 热力图右侧部分，生成一个 token 时总是对它的上一个 token 有较强的注意力
[![非幻觉场景 2: 每个生成 token 在 system prompt 区域的注意力曲线](12-attention12.png)](12-attention12.png "非幻觉场景 2: 每个生成 token 在 system prompt 区域的注意力曲线")非幻觉场景 2: 每个生成 token 在 system prompt 区域的注意力曲线

#### [](#幻觉场景 "幻觉场景")幻觉场景

[![幻觉场景 2: 测试用例，输出幻觉](13-attention13.png)](13-attention13.png "幻觉场景 2: 测试用例，输出幻觉")幻觉场景 2: 测试用例，输出幻觉
[![幻觉场景 2: 热力图，system prompt 区域没有明显的注意力峰值](14-attention14.png)](14-attention14.png "幻觉场景 2: 热力图，system prompt 区域没有明显的注意力峰值")幻觉场景 2: 热力图，system prompt 区域没有明显的注意力峰值
[![幻觉场景 2: 每个生成 token 在 system prompt 区域的注意力曲线](15-attention15.png)](15-attention15.png "幻觉场景 2: 每个生成 token 在 system prompt 区域的注意力曲线")幻觉场景 2: 每个生成 token 在 system prompt 区域的注意力曲线

## [](#Vibe-Coding-的正确姿态：要当老师，而不是学生 "Vibe Coding 的正确姿态：要当老师，而不是学生")Vibe Coding 的正确姿态：要当老师，而不是学生
...
### [](#❌-错误的方式（学生心态）： "❌ 错误的方式（学生心态）：")❌ 错误的方式（学生心态）：

-   “这个幻觉检测该怎么做？”
-   “为什么结果不对？你帮我看看”
-   “我也不知道这方法行不行，先这么干试试吧”

### [](#✅-正确的方式（老师心态）： "✅ 正确的方式（老师心态）：")✅ 正确的方式（老师心态）：
...
_整个探索过程，从建立可视化到编码实现整个方案，只用了 2 个小时。这就是正确的 Vibe Coding 方式带来的效率提升。我司的面试题给候选人的时限也是 2 个小时。_
