...
> 发布时间: 2024-08-01T20:12:34
> 原文链接: https://huggingface.co/blog/1_58_llm_extreme_quantization

---
> **Authors:** [Mohamed Mekkouri](/medmekk) (@medmekk), [Marc Sun](/marcsun13) (@marcsun13), [Leandro von Werra](/lvwerra) (@lvwerra), [Pedro Cuenca](/pcuenq) (@pcuenq), [Omar Sanseviero](/osanseviero) (@osanseviero), [Thomas Wolf](/thomwolf) (@thomwolf)

This article is also available in Chinese [简体中文](/blog/zh/1_58_llm_extreme_quantization).
...
![The new computation paradigm of BitNet b1.58](images/img_013.png)
...
![Energy consumption of BitNet b1.58 compared to LLaMA](images/img_014.png)
...
![The architecture of BitNet with BitLinear layers](images/img_015.png)
...
$W\_q = \text{clamp}\_{\[-1,1]}(\text{round}$

Wdequantized\=Wq∗scalew W_{dequantized} = W_q\*scale_w Wdequantized​\=Wq​∗scalew​
...
$X\_q = \text{clamp}\_{\[-128,127]}(\text{round}$

Xdequantized\=Xq∗scalex X_{dequantized} = X_q \* scale_x Xdequantized​\=Xq​∗scalex​
...
W\=[0.8−0.51.2−1.50.4−0.91.3−0.70.2] W = \\begin{bmatrix} 0.8 & -0.5 & 1.2 \\\\ -1.5 & 0.4 & -0.9 \\\\ 1.3 & -0.7 & 0.2 \\end{bmatrix} W\=​0.8−1.51.3​−0.50.4−0.7​1.2−0.90.2​​

**Step 1: Compute the Scale for Weights**

Using the formula:

scalew\=11nm∑ij∣Wij∣ scale_w = \\frac{1}{\\frac{1}{nm} \\sum_{ij} |W_{ij}|} scalew​\=nm1​∑ij​∣Wij​∣1​

we calculate the average absolute value of ( W ):

1nm∑ij∣Wij∣\=19(0.8+0.5+1.2+1.5+0.4+0.9+1.3+0.7+0.2)\=19(7.5)\=0.8333 \\frac{1}{nm} \\sum_{ij} |W_{ij}| = \\frac{1}{9}(0.8 + 0.5 + 1.2 + 1.5 + 0.4 + 0.9 + 1.3 + 0.7 + 0.2) = \\frac{1}{9}(7.5) = 0.8333 nm1​∑ij​∣Wij​∣\=91​(0.8+0.5+1.2+1.5+0.4+0.9+1.3+0.7+0.2)\=91​(7.5)\=0.8333

Now, the scale factor is:

scalew\=10.8333≈1.2 scale_w = \\frac{1}{0.8333} \\approx 1.2 scalew​\=0.83331​≈1.2

**Step 2: Quantize the Weight Matrix**

Using the formula:

$W_q = \text{clamp}_{[-1, 1]}(\text{round}(W \times$

We first scale the weights by scalew≈1.2 scale_w \\approx 1.2 scalew​≈1.2:

W×scalew\=[0.8×1.2−0.5×1.21.2×1.2−1.5×1.20.4×1.2−0.9×1.21.3×1.2−0.7×1.20.2×1.2]\=[0.96−0.61.44−1.80.48−1.081.56−0.840.24] W \\times scale_w = \\begin{bmatrix} 0.8 \\times 1.2 & -0.5 \\times 1.2 & 1.2 \\times 1.2 \\\\ -1.5 \\times 1.2 & 0.4 \\times 1.2 & -0.9 \\times 1.2 \\\\ 1.3 \\times 1.2 & -0.7 \\times 1.2 & 0.2 \\times 1.2 \\end{bmatrix} = \\begin{bmatrix} 0.96 & -0.6 & 1.44 \\\\ -1.8 & 0.48 & -1.08 \\\\ 1.56 & -0.84 & 0.24 \\end{bmatrix} W×scalew​\=​0.8×1.2−1.5×1.21.3×1.2​−0.5×1.20.4×1.2−0.7×1.2​1.2×1.2−0.9×1.20.2×1.2​​\=​0.96−1.81.56​−0.60.48−0.84​1.44−1.080.24​​

Next, we round the values and clamp them to the range [−1,1] [-1, 1] [−1,1]:

Wq\=[1−11−10−11−10] W_q = \\begin{bmatrix} 1 & -1 & 1 \\\\ -1 & 0 & -1 \\\\ 1 & -1 & 0 \\end{bmatrix} Wq​\=​1−11​−10−1​1−10​​

**Step 3: Dequantize the Weights**

Finally, we dequantize the weights using:

Wdequantized\=Wq×scalew W_{dequantized} = W_q \\times scale_w Wdequantized​\=Wq​×scalew​

Substituting scale_w, we get:

Wdequantized\=[1×1.2−1×1.21×1.2−1×1.20×1.2−1×1.21×1.2−1×1.20×1.2]\=[1.2−1.21.2−1.20−1.21.2−1.20] W_{dequantized} = \\begin{bmatrix} 1 \\times 1.2 & -1 \\times 1.2 & 1 \\times 1.2 \\\\ -1 \\times 1.2 & 0 \\times 1.2 & -1 \\times 1.2 \\\\ 1 \\times 1.2 & -1 \\times 1.2 & 0 \\times 1.2 \\end{bmatrix} = \\begin{bmatrix} 1.2 & -1.2 & 1.2 \\\\ -1.2 & 0 & -1.2 \\\\ 1.2 & -1.2 & 0 \\end{bmatrix} Wdequantized​\=​1×1.2−1×1.21×1.2​−1×1.20×1.2−1×1.2​1×1.2−1×1.20×1.2​​\=​1.2−1.21.2​−1.20−1.2​1.2−1.20​​

Example 2: Activation Matrix Quantization

Let the activation matrix ( X ) be:

X\=[1.0−0.60.7−0.90.4−1.20.8−0.50.3] X = \\begin{bmatrix} 1.0 & -0.6 & 0.7 \\\\ -0.9 & 0.4 & -1.2 \\\\ 0.8 & -0.5 & 0.3 \\end{bmatrix} X\=​1.0−0.90.8​−0.60.4−0.5​0.7−1.20.3​​
...
scale\=[1271.01271.21270.8]\=[127105.83158.75] \\text{scale} = \\begin{bmatrix} \\frac{127}{1.0} \\\\ \\frac{127}{1.2} \\\\ \\frac{127}{0.8} \\end{bmatrix} = \\begin{bmatrix} 127 \\\\ 105.83 \\\\ 158.75 \\end{bmatrix} scale\=​1.0127​1.2127​0.8127​​​\=​127105.83158.75​​
...
X×scale\=[1.0×127−0.6×1270.7×127−0.9×105.830.4×105.83−1.2×105.830.8×158.75−0.5×158.750.3×158.75]\=[127−76.288.9−95.242.3−127127−79.447.6] X \\times \\text{scale} = \\begin{bmatrix} 1.0 \\times 127 & -0.6 \\times 127 & 0.7 \\times 127 \\\\ -0.9 \\times 105.83 & 0.4 \\times 105.83 & -1.2 \\times 105.83 \\\\ 0.8 \\times 158.75 & -0.5 \\times 158.75 & 0.3 \\times 158.75 \\end{bmatrix} = \\begin{bmatrix} 127 & -76.2 & 88.9 \\\\ -95.2 & 42.3 & -127 \\\\ 127 & -79.4 & 47.6 \\end{bmatrix} X×scale\=​1.0×127−0.9×105.830.8×158.75​−0.6×1270.4×105.83−0.5×158.75​0.7×127−1.2×105.830.3×158.75​​\=​127−95.2127​−76.242.3−79.4​88.9−12747.6​​

Round the values and clamp them to the range [−128,127][-128, 127] [−128,127]:

Xq\=[127−7689−9542−127127−7948] X_q = \\begin{bmatrix} 127 & -76 & 89 \\\\ -95 & 42 & -127 \\\\ 127 & -79 & 48 \\end{bmatrix} Xq​\=​127−95127​−7642−79​89−12748​​

**Step 3: Dequantize the Activations**

Finally, dequantize the activations using:

Xdequantized\=Xq×1scale X_{dequantized} = X_q \\times \\frac{1}{\\text{scale}} Xdequantized​\=Xq​×scale1​

Substituting the scales:

Xdequantized\=[127×1127−76×112789×1127−95×1105.8342×1105.83−127×1105.83127×1158.75−79×1158.7548×1158.75]\=[1.0−0.60.7−0.90.4−1.20.8−0.50.3] X_{dequantized} = \\begin{bmatrix} 127 \\times \\frac{1}{127} & -76 \\times \\frac{1}{127} & 89 \\times \\frac{1}{127} \\\\ -95 \\times \\frac{1}{105.83} & 42 \\times \\frac{1}{105.83} & -127 \\times \\frac{1}{105.83} \\\\ 127 \\times \\frac{1}{158.75} & -79 \\times \\frac{1}{158.75} & 48 \\times \\frac{1}{158.75} \\end{bmatrix} = \\begin{bmatrix} 1.0 & -0.6 & 0.7 \\\\ -0.9 & 0.4 & -1.2 \\\\ 0.8 & -0.5 & 0.3 \\end{bmatrix} Xdequantized​\=​127×1271​−95×105.831​127×158.751​​−76×1271​42×105.831​−79×158.751​​89×1271​−127×105.831​48×158.751​​​\=​1.0−0.90.8​−0.60.4−0.5​0.7−1.20.3​​
...
![Pre-training plots without (blue) & with (green) layer normalisation](images/img_016.png)
...
![Fine-tuning plot compared to pre-training plot](images/img_017.png)
...
![Random weights distribution (2 merged stds)](images/img_018.png)

Random weights distribution (2 merged stds)

![Pre-trained Llama3 weights distribution](images/img_019.png)

Pre-trained Llama3 weights distribution

And the scale values for the two distributions are, respectively :

![Random weights scales distribution](images/img_020.png)

Random weights scales distribution

![Pre-trained Llama3 weights distribution](images/img_021.png)
...
-   For $\text{scale} = 50.25$, w.abs().mean()\=0.0199 w.abs().mean() = 0.0199 w.abs().mean()\=0.0199, leading to std\=0.025\\text{std} = 0.025 std\=0.025 which matches our first standard deviation. The formula used to derive the std is based on the expectation of the half-normal distribution of ∣w∣ |w| ∣w∣:
    E(∣w∣)\=std(w)⋅2π \\mathbb{E}(|w|) = \\text{std}(w) \\cdot \\sqrt{\\frac{2}{\\pi}} E(∣w∣)\=std(w)⋅π2​​
-   For $\text{scale} = 402$, w.abs().mean()\=0.0025 w.abs().mean() = 0.0025 w.abs().mean()\=0.0025, leading to $\text{std} = 0.00325$
...
![Fine-tuning plot with lambda = 0.25->0.5->0.75->1](images/img_022.png)
...
![Fine-tuning plot with lambda = min(2\*training_step/total_training_steps, 1)](images/img_023.png)

Fine-tuning plot with lambda = min(2\*training\_step/total\_training\_steps, 1)
...
![Fine-tuning plot with warmup quantization on Fineweb-edu](images/img_024.png)
...
![Exponential scheduler for different k values](images/img_025.png)
...
![Fine-tuning plots with exponential scheduler](images/img_026.png)
...
![Sigmoid scheduler for different k values](images/img_027.png)

Sigmoid scheduler for different k values

We ran 5 experiments this time with k in \[15, 20, 25, 40, 100] :

![Finetuning plots with sigmoid scheduler](images/img_028.png)
...
![Different Pre-training plots with different learning rates](images/img_029.png)
...
![Metrics evaluations during the training for different lrs](images/img_030.png)

Metrics evaluations during the training for different lrs

and the average score looks like :

![Average evaluation during the training for different lrs](images/img_031.png)
...
![Smoll LLm fine-tuning experiment with & without warmup quantization](images/img_032.png)
...
![Metrics comparison with Llama models](images/img_033.png)
...
![Metrics comparison with Llama models for the model trained on 100B tokens](images/img_034.png)
...
![Tiled Matrix multiplication illustration](images/img_035.png)
...
![Triton kernel compared to torch.compile](images/img_036.png)
...
![Bitblas benchmark](images/img_037.png)
