...
export interface DeepwikiLitenextMetadata {
  title: string;
}

export interface DeepwikiLitenextConvertResult {
  markdown: string;
  imagesToDownload: DeepwikiLitenextImageDownload[];
  metadata: DeepwikiLitenextMetadata;
  stats: {
...
export function convertDeepwikiLitenextHtml(
  contentHtml: string,
  mermaidSources: string[],
  opts: ConvertOpts,
): DeepwikiLitenextConvertResult {
  ...
  const imagesToDownload: DeepwikiLitenextImageDownload[] = [];
  let imgCounter = 0;
...
function emptyResult(opts: ConvertOpts, reason: string): DeepwikiLitenextConvertResult {
  const md =
...
function makeTurndown(): TurndownService {
  ...
  // class="language-mermaid" on those inside `convertDeepwikiLitenextHtml`).
