...
export function cmdPlan(candidatesJsonPath: string, paths: Paths = defaultPaths): PlanResult {
  ...
  const result: PlanResult = { added: 0, dedupedByUrl: 0, dedupedById: 0, skippedDone: 0, totalPending: 0 };
...
function usage(): never {
  console.error(`usage:
  tsx ingest_batch.ts plan <candidates.json>
...
function main(): void {
  ...
  switch (cmd) {
    case "plan": {
      if (!rest[0]) usage();
      const r = cmdPlan(resolve(rest[0]));
      console.log(
