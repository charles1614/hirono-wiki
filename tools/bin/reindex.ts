 ...
 * Meta/index.md counts. Also updates `refs` / `tier` / `source_count` in
 ...
 *
 *   npx tsx reindex.ts            # full pass, write changes
 *   npx tsx reindex.ts --dry-run  # print what would change, no writes
 */
...
function renderSourcesIndex(docs: DocMeta[]): string {
  ...
  lines.push("");
  lines.push("Every ingested source page, grouped by year. Regenerated by `tools/reindex.ts`.");
  lines.push("");
...
function renderEntitiesIndex(
  ...
  const entities = docs.filter((d) => d.bucket === "Entities");
...
function renderTopicsIndex(
  ...
  const topics = docs.filter((d) => d.bucket === "Topics")
  ...
  lines.push("");
  lines.push("Every synthesis page. Regenerated by `tools/reindex.ts`.");
  lines.push("");
...
function renderIndexOverview(
  ...
  const sources = docs.filter((d) => d.bucket === "Sources").length;
  const entities = docs.filter((d) => d.bucket === "Entities");
  const active = entities.filter((d) => !d.repo_path.includes("/_seen/")).length;
  const seen = entities.length - active;
  const topics = docs.filter((d) => d.bucket === "Topics").length;
...
export function computeReindex(repoRoot: string): ReindexResult {
  ...
  const indexFiles = [
    { path: "Meta/index.md", content: renderIndexOverview(virtualDocs) },
    { path: "Meta/index-sources.md", content: renderSourcesIndex(virtualDocs) },
    { path: "Meta/index-entities.md", content: renderEntitiesIndex(virtualDocs, refs) },
    { path: "Meta/index-topics.md", content: renderTopicsIndex(virtualDocs, sourceCounts) },
  ];
  return { pending, indexFiles, refs };
}

function apply(repoRoot: string, result: ReindexResult, dryRun: boolean): void {
  // Entities/topics frontmatter updates + renames
