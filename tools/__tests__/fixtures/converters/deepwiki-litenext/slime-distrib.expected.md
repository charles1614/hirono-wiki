...
```python
# slime/ray/placement_group.py:41-76
def _create_placement_group(num_gpus):
    bundles = [{"GPU": 1, "CPU": 1} for _ in range(num_gpus)]
    pg = placement_group(bundles, strategy="PACK")
    ray.get(pg.ready())

    # Discover physical GPU IDs via ephemeral InfoActor instances
    info_actors = []
    ...
    # Sort bundles by (node IP, GPU ID) for deterministic ordering
...
The function supports four modes through command-line flags:
...
```python
# slime/ray/actor_group.py:79-90
...
```python
# slime/ray/actor_group.py:94-106
...
The synchronous loop (`train.py:9-100`) follows a strict sequential pattern:
...
```python
# slime/utils/health_monitor.py:150-158
try:
...
```mermaid
graph TB
    hmCheck["HealthMonitor: health_generate.remote()"]
    hmFail["Engine: Timeout or Exception"]
    hmKill["HealthMonitor: _kill_engine(id)all_engines id = None"]
    rsRecover["RolloutServer: recover_rollout_engines()"]
    rsStart["EngineGroup: start_engines()"]
    rsInit["ray.get(init handles)"]
    rsOffload["release/resume_memory if offload mode"]

    hmCheck --> hmFail --> hmKill
    hmKill --> rsRecover --> rsStart --> rsInit --> rsOffload
```
