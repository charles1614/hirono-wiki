...
![OAS6b8BCNofkKdx8Ujjc5n1xncc](sspai-img-001.jpg)
...
![SYRgbiBKHoujFxx86OJcRgELn5e](sspai-img-002.png)
...
![EzfBb4jsKonMwMxS6s1ceVqTnae](sspai-img-003.png)

开通 R2 服务后，点击右上角「创建存储桶」按钮进行创建。

![DPvrbIwheo7mIvxB0K6cRp19nVb](sspai-img-004.png)
...
![DTXvbOxaqo5BfdxroyvcFs4dnue](sspai-img-005.png)
...
![ZGCgbtc2RoEBU0xnrTGcIAbwnfc](sspai-img-006.png)

首先我们需要打开「R2.dev 子域」，这是为了后续访问图片时需要的公网地址，点击「允许访问」，并按照提示输入「allow」即可开启。

![G1iLbdJ0joB68qx3Bl2cj5H8nTQ](sspai-img-007.png)
...
![Z67NbjAK9oYBIAxaFZncFWClnRe](sspai-img-008.png)

输入自己想绑定的域名，如 `yu-r2-test.pseudoyu.com`，点击继续。

![MsGXbzaHIoOovvxmnVCc0BMgnvb](sspai-img-009.png)![TdO0bptycoXFWyxGewucqrDGnie](sspai-img-010.png)

连接域，并等待 DNS 解析生效即可。

![SyTVbUjWrop0eSxdZ4lcf8NWnkc](sspai-img-011.png)

完成后 bucket 状态处「公共 URL 访问」显示“已允许”，且「域」显示为我们刚自定义的域名即配置成功。

#### 配置 Bucket 访问 API

![APacbU5pWoT4jzxHALgcZtwKnHe](sspai-img-012.png)
...
![M7U9b8iWmoPIqSxC2gjcSGisnnc](sspai-img-013.png)![QeiDbUeweofzOsxanjsc91v6nPx](sspai-img-014.png)

回到 R2 主页面，点击右上角「管理 R2 API 令牌」，进入后点击「创建 API 令牌」。

![QYF2bnVgVoJ3WIx3WJ9corX7ndd](sspai-img-015.png)

输入令牌名称，「权限」选择“对象读和写”并将此 API 指定之前所创建的 Bucket，这样最小化权限也保障数据安全，其余选项保持默认即可。

![SRzmbDEfLonpwqxazvIcTUL1nsc](sspai-img-016.png)
...
![FMwabhdhGojlwtxiIyScRHcVnCf](sspai-img-017.png)

在「插件设置」中选择安装，「图床设置」中就新增了 Amazon S3 选项。点击进入配置选项。

![QVQrbPbu8oYQ8txPpNVc6QjOnvd](sspai-img-018.png)
...
![JYLsbRdF4omhFPxGgsJcxQTjnrb](sspai-img-019.png)

完成上述配置后，我们就可以在「上传区」直接拖入文件进行图片上传了，如上传后显示无误则为配置成功，生成的链接会自动在系统剪贴板中，直接在需要的地方粘贴即可。

![RA1sbFL1LoYb5wx7jp2cS8srnyh](sspai-img-020.png)
...
![RlJibimTdoEoWXxFkbvclcPknbb](sspai-img-021.png)
...
![M4vbbrAu8oaYSPxlnQscEsu6npb](sspai-img-022.png)
...
![G3pybPmywoY6hZxEnscccgdjnXe](sspai-img-023.png)

首先通过 GitHub 授权登录 [WebP Cloud](https://dashboard.webp.se/) 平台。

![XTC4bMTC7oXDy3xEDZCcIhp9nbe](sspai-img-024.png)

页面非常直观，主要显示了当前 Plan 下的 Free Quota 和额外 Quota 的数据，以及一些用量统计。

点击「Create Proxy」按钮可以添加配置。

![OTuNbicfooC964xif2Tcz2kZnGg](sspai-img-025.png)
...
![Vre4bBcQgoT1ydxnoYTcK3BMn8b](sspai-img-026.png)

图中 Basic info 部分「Visitor」下显示的以 `xxx.webp.li` 格式即为我们的代理地址。

例如我们之前通过 PicGo 上传到 R2 的文件 [yu-r2-test.pseudoyu.com/new\_mbp\_setup.jpg](https://yu-r2-test.pseudoyu.com/new_mbp_setup.jpg) 则可以用 [dc84642.webp.li/new\_mbp\_setup.jpg](https://dc84642.webp.li/new_mbp_setup.jpg) 这一链接进行访问。
...
![FTbzbPw7robMWLxI8V4croDenHh](sspai-img-027.png)
