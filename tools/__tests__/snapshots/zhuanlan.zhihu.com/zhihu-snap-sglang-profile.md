# SGLang Server交互式Profiling的一些技巧(PyTorch & Nsys)

> 作者: unknown
> 发布时间: 编辑于 2026-01-09 16:25・湖北
> 原文链接: https://zhuanlan.zhihu.com/p/1992613341697438794
...
在学习SGLang Docs的相应内容后，我了解到SGLang现在已经提供Profile a server with HTTP API endpoints的方式来灵活地控制Profile with [PyTorch Profiler](https://zhida.zhihu.com/search?content_id=268737839&content_type=Article&match_order=1&q=PyTorch+Profiler&zhida_source=entity). 然而文档对于 Profile with Nsys 只介绍了设置 delay 和 duration 的做法。意味着需要在SGLang Server启动时就开启profile, 同时提前设定好profile的时机，这种方法确实是不够灵活。

因此，本文的内容主要包括三部分：

-   针对Pytorch Profile, 介绍文档中Profile a server with HTTP API endpoints的做法
-   针对[Nsight Profile](https://zhida.zhihu.com/search?content_id=268737839&content_type=Article&match_order=1&q=Nsight+Profile&zhida_source=entity), 介绍不同于文档中的交互式profile做法
-   针对多rank profile的结果, 介绍如何在nsight system或[perfetto](https://zhida.zhihu.com/search?content_id=268737839&content_type=Article&match_order=1&q=perfetto&zhida_source=entity)中方便比较各rank的小技巧

此外由于个人认识有限，大佬们可能已经有了更好的profile方法，欢迎批评指正。多多交流
...
SGLang 在 server 运行期间提供了两组 HTTP API：/start\_profile 与 /end\_profile . 这使得我们不需要重启服务，也不需要提前设定profiling 时间窗，你可以在server跑起来后，随时开始抓取trace，再在合适时机结束，从而更精准地对齐某类 workload pattern.
...
-   output\_dir: 确定输出文件夹. 如果没有设定，基于SGLANG\_TORCH\_PROFILER\_DIR环境变量或者默认/tmp.
-   start\_step: 什么时候开始profile, 用于跳过warm-up阶段.
-   num\_steps: 需要profile的step数. 如果不设定则会一直profiling.
-   activities: 需要profile的活动. 包括CPU, GPU, MEM和RPD. 默认是CPU和GPU.
-   merge\_profiles: 自动合并所有rank的分析结果，默认为false
-   profile\_prefix: 给trace文件添加指定前缀，用于区分.

**生成的trace文件命名格式:**

{$profile\_prefix}-{profile\_id}-TP-{tp}-DP-{dp}-PP-{pp}-EP-{ep}.trace.json.gz

**合并文件格式：**

merged-{profile\_id}.trace.json.gz
...
手动停止server的profile，如果在start\_profile中已经设定了num\_steps, server将在完成设定stepd的profile会停止.
...
只想抓取10个step的trace信息，那么在另一个终端start\_profile
...
在另一个终端手动发送start\_profile和end\_profile

```bash
curl -X POST http://127.0.0.1:30000/start_profile

# 这段时间把你想看的 workload 跑出来
# 比如压测、或者复现某个线上请求形态
...
SGLang Docs在该部分，只介绍了通过设定delay与duration参数的方式来profile sglang server.

这意味着我们需要在server启动时便设定好开始profile的时间与持续时间(秒).
...
### 3.1 基于nsys start与nsys stop交互式profile

在启动sglang server时，加入nsys profile的额外参数start-later, 这将使得nsys profile不立刻进行.

```bash
nsys profile --trace-fork-before-exec=true --cuda-graph-trace=node -o sglang.out \
  --start-later \
  python3 -m sglang.launch_server --model-path meta-llama/Llama-3.1-8B-Instruct --disable-radix-cache
```

在另一个终端，我们可以通过nsys session list查看正在运行的nsys profile session ID.
...
在秋招时被问到过，抓取多rank的trace后，如果想可视化比较各rank的kernel执行时间。如何方便观察呢？
