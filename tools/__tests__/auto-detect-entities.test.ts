...
function seedSource(root: string, slug: string, body: string): void {
  writeFile(root, `Sources/2026/${slug}.md`, body);
...
test("autoDetect prepare mode: writes prompt package + lists existing entities", () => {
  ...
  try {
    ...
    writeFile(root, "Entities/DeepSeek.md", "---\ntype: entity\n---\n\n# DeepSeek\n");
    writeFile(root, "Entities/_seen/Hopper.md", "---\ntype: entity\n---\n\n# Hopper\n");
...
test("autoDetect apply: creates _seen/ stubs atomically + log entry", () => {
  ...
  try {
    ...
    const faStub = readFileSync(join(root, "Entities/_seen/FlashAttention.md"), "utf8");
    ...
    const hopperStub = readFileSync(join(root, "Entities/_seen/Hopper.md"), "utf8");
    assert.ok(hopperStub.includes("# Hopper"));

    // Existing entity should not be re-created
    const deepseek = readFileSync(join(root, "Entities/DeepSeek.md"), "utf8");
...
test("autoDetect: throws on missing raw archive", () => {
  const root = makeRepo();
  try {
    writeFile(root, `Sources/2026/test-slug.md`, `---\ntype: source\n---\n\nBody.\n`);
