...
| Tier-2 batch curation (propose-curation → apply-queue): one Sonnet pass over health-check findings, operator reviews queue, dispatcher executes approved items via existing atomic CLIs | [`Meta/operator-workflows.md`](Meta/operator-workflows.md) §13 Tier-2 batch curation |
